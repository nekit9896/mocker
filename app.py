...
if (
    ...
    app.run(debug=True, port=8082, host='0.0.0.0')  # указываем порт мокера и хост, если не указать хост то мокер
    # внутри докера будет резолвится сам на себя создавая петлю
